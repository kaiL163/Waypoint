...
import { changesBetween } from './planDiff'
...
const isAtRisk=(request:ServiceRequest,plan:Plan|null)=>{const stop=stopFor(plan,request.id);return Boolean(stop&&timeMinutes(request.windowEnd)-timeMinutes(stop.plannedArrival)<=30)}
...
export default function App(){
  ...
  const loadDemo=(scenario?:DemoScenarioInfo)=>run('Загружаем демо-данные...',async()=>{const loaded=await api.demo(scenario?.id);const title=scenario?.title??demos.find(item=>item.id==='standard')?.title??'Демо';setData(loaded);setPlan(null);setBaseline(null);setChanges([]);setSelectedRequest(null);setSelectedEngineer(null);setDemoMenuOpen(false);setNotice(`${title}: ${loaded.engineers.length} инженеров, ${loaded.requests.length} заявок.`)})
  const upload=(files:FileList|null)=>{if(!files?.length)return;void run('Загружаем данные...',async()=>{const loaded=await api.upload(files);setData(loaded);setPlan(null);setBaseline(null);setChanges([]);setSelectedEngineer(null);setSelectedRequest(null);setNotice(`Загружено: ${loaded.engineers.length} инженеров, ${loaded.requests.length} заявок.`)});if(fileRef.current)fileRef.current.value=''}
  const build=()=>{if(!data)return;void run('Строим план...',async()=>{const [result,basePlan]=await Promise.all([api.optimize(data,'optimized'),api.optimize(data,'baseline')]);setPlan(result);setBaseline(basePlan);setChanges([]);setShowAllRoutes(true);setNotice('План построен. Маршруты на карте, сводка — под картой.')})}
  const rebuild=()=>{if(!data)return;void run('Перестраиваем маршруты...',async()=>{const result=await api.optimize(data,'optimized');const basePlan=await api.optimize(data,'baseline');setChanges(plan?changesBetween(plan,result,data):[]);setPlan(result);setBaseline(basePlan);setShowAllRoutes(true);setNotice('План обновлён. Изменения показаны под картой.')})}
  const applyPlanningEvent=(event:PlanningEvent)=>{if(!data)return;const mapEvent=():ReplanEvent=>{if(event.type==='urgent')return {type:'urgent_request',input:event.input};if(event.type==='cancel')return {type:'cancel_request',requestId:event.requestId};return {type:'engineer_unavailable',engineerId:event.engineerId}};const message=event.type==='urgent'?'Срочная заявка добавлена. План перестроен, изменения показаны под картой.':event.type==='cancel'?`Заявка #${event.requestId} отменена. План перестроен.`:'Инженер отмечен недоступным. План перестроен с учётом изменений.';void run('Перестраиваем маршруты...',async()=>{const result=await api.replan(data,mapEvent(),plan);const basePlan=await api.optimize(result.dataset,'baseline');setData(result.dataset);setPlan(result.plan);setBaseline(basePlan);setChanges(result.changes);setShowAllRoutes(true);setNotice(message);setEventOpen(false);if(event.type==='urgent'){const urgent=result.dataset.requests.find(r=>r.priority==='urgent'&&!data.requests.some(old=>old.id===r.id));setSelectedEngineer(null);if(urgent)setSelectedRequest(urgent.id)}if(event.type==='cancel')setSelectedRequest(null);if(event.type==='unavailable')setSelectedEngineer(null)})}
  return <main className="app-shell">
    <header className="app-header"><div className="brand"><img className="brand-logo" src="/waypoint-logo.png" alt="Waypoint"/><span>Диспетчерская панель</span></div><div className="header-center"><CalendarDays size={16}/><span>{new Intl.DateTimeFormat('ru-RU',{day:'numeric',month:'long',year:'numeric'}).format(new Date())}</span><span className="plan-status"><i className={plan?'ready':''}/>{plan?'План построен':data?'Данные загружены':'Нет данных'}</span></div><div className="header-actions"><input ref={fileRef} className="sr-only" type="file" accept=".json,.csv" multiple onChange={e=>upload(e.target.files)}/><button className="button subtle" onClick={()=>fileRef.current?.click()}><FileUp size={16}/>Загрузить данные</button><div className="demo-picker"><button className="button subtle" aria-expanded={demoMenuOpen} onClick={()=>setDemoMenuOpen(open=>!open)}>Демо-данные<ChevronRight size={14}/></button>{demoMenuOpen&&<div className="demo-menu" role="menu">{(demos.length?demos:[{id:'standard',title:'Демо 1 · Рабочий день',description:'Загрузка с сервера'}]).map(scenario=><button key={scenario.id} role="menuitem" onClick={()=>loadDemo(scenario)}><b>{scenario.title}</b><small>{scenario.description}</small></button>)}</div>}</div><button className="button subtle norms-button" onClick={()=>setNormsOpen(true)}><BookOpen size={16}/>Нормативы</button><button className="button secondary" disabled={!data||!!loading} onClick={()=>setEventOpen(true)}><Plus size={16}/>Событие</button><button className="button secondary" disabled={!plan||!!loading} onClick={rebuild}><RouteIcon size={16}/>Перестроить</button><button className="button primary" disabled={!data||!!loading} onClick={build}><Sparkles size={16}/>Построить план</button></div></header>
...
function DispatchMap({data,plan,selectedEngineer,selectedRequest,showAllRoutes,onEngineer,onRequest}:{data:Dataset;plan:Plan|null;selectedEngineer:string|null;selectedRequest:string|null;showAllRoutes:boolean;onEngineer:(id:string)=>void;onRequest:(id:string)=>void}){
  ...
  return <div className="map-section"><div className="map-wrap"><MapContainer key={data.engineers.map(e=>e.id).join('|')+data.requests.length} center={center} zoom={12} scrollWheelZoom attributionControl={false} className="leaflet-map"><MapResizeSync/><TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/>
    {plan?.routes.map(route=>{const engineer=data.engineers.find(e=>e.id===route.engineerId),selected=selectedEngineer===route.engineerId;if(!engineer||!selected&&!showAllRoutes)return null;const locations=[engineer.startLocation,...route.stops.map(s=>data.requests.find(r=>r.id===s.requestId)?.location).filter((x):x is ServiceRequest['location']=>Boolean(x))];return <Polyline key={route.engineerId} positions={locations.map(p=>[p.lat,p.lng] as [number,number])} pathOptions={{color:colorFor(data.engineers,route.engineerId),weight:selected?7:3,opacity:selected?.96:selectedEngineer?.14:.42}} eventHandlers={{click:()=>onEngineer(route.engineerId)}}/>})}
...
function ConstraintChecklist({request,engineer,stop,reason}:{request:ServiceRequest;engineer:Engineer|undefined;stop:ReturnType<typeof stopFor>;reason:string|undefined}){const windowReserve=stop?timeMinutes(request.windowEnd)-timeMinutes(stop.plannedArrival):null;const items=engineer?[['Навык',engineer.skills.includes(request.requiredSkill),request.requiredSkill],['Транспорт',!request.requiredTransport||engineer.transport===request.requiredTransport,request.requiredTransport?transportLabel[request.requiredTransport]:'Не ограничен'],['Окно',Boolean(stop&&timeMinutes(stop.plannedStart)>=timeMinutes(request.windowStart)&&timeMinutes(stop.plannedEnd)<=timeMinutes(request.windowEnd)),stop?`Запас ${Math.max(0,windowReserve??0)} мин`:'Не рассчитано'],['Смена',Boolean(stop&&timeMinutes(stop.plannedEnd)<=timeMinutes(engineer.shiftEnd)),`${engineer.shiftStart}–${engineer.shiftEnd}`]]:[];return <section className="constraints"><h3>Проверка ограничений</h3>{items.length?items.map(([label,ok,note])=><div key={String(label)} className={ok?'ok':'fail'}><Check size={13}/><span><b>{label}</b><small>{note}</small></span></div>):<div className="constraint-empty"><AlertTriangle size={13}/><span>{reason?'Подходящего назначения нет — см. причину ниже.':'Постройте план, чтобы проверить ограничения.'}</span></div>}</section>}
function RequestDrawer({request,data,plan,close}:{request:ServiceRequest;data:Dataset;plan:Plan|null;close:()=>void}){const stop=stopFor(plan,request.id),engineer=data.engineers.find(e=>e.id===stop?.engineerId),reason=reasonFor(plan,request.id),atRisk=isAtRisk(request,plan),reserve=stop?timeMinutes(request.windowEnd)-timeMinutes(stop.plannedArrival):null;return <aside className="drawer"><button className="close" onClick={close} aria-label="Закрыть"><X size={18}/></button><span className={'badge '+(reason?'unassigned':request.priority==='urgent'?'urgent':stop?'planned':'neutral')}>{reason?'Не назначена':request.priority==='urgent'?'Срочная':stop?'Запланирована':'Не запланирована'}</span><h2>Заявка #{request.id}</h2><p className="address"><MapPin size={16}/>{request.location.address}</p><div className="detail-grid"><div><small>Временное окно</small><b>{request.windowStart}–{request.windowEnd}</b></div><div><small>Длительность</small><b>{request.durationMinutes} минут</b></div><div><small>Тип работ</small><b>{request.requiredSkill}</b></div><div><small>Транспорт</small><b>{request.requiredTransport?transportLabel[request.requiredTransport]:'Любой'}</b></div><div><small>Приоритет</small><b>{request.priority==='urgent'?'Срочный':'Обычный'}</b></div><div><small>Статус</small><b>{reason?'Не назначена':stop?'Запланирована':'Не запланирована'}</b></div></div>{atRisk&&<div className="risk-note"><AlertTriangle size={15}/><span><b>Риск опоздания</b><small>Запас до конца окна: {Math.max(0,reserve??0)} мин</small></span></div>}<ConstraintChecklist request={request} engineer={engineer} stop={stop} reason={reason}/>{reason?<div className="reason"><AlertTriangle size={18}/><div><b>Почему не назначена</b><p>{reason}</p></div></div>:stop&&engineer?<><div className="assign"><small>НАЗНАЧЕНИЕ</small><div className="assign-person"><span className="avatar" style={{background:colorFor(data.engineers,engineer.id)}}>{engineer.name.split(' ').map(x=>x[0]).slice(0,2).join('')}</span><div><b>{engineer.name}</b><p>Остановка {stop.order} в маршруте</p></div></div><div className="time-list"><span>Прибытие <b>{stop.plannedArrival}</b></span><span>Начало работ <b>{stop.plannedStart}</b></span><span>Завершение <b>{stop.plannedEnd}</b></span></div></div><div className="reason success"><Check size={18}/><div><b>Почему выбран {engineer.name}</b><p>Есть навык «{request.requiredSkill}», подходит транспорт, прибытие укладывается во временное окно, работа завершается до конца смены.</p></div></div></>:<div className="reason neutral"><p>Постройте план, чтобы увидеть назначение и объяснение.</p></div>}</aside>}
...
function PlanningEventDialog({data,onClose,onSubmit}:{data:Dataset;onClose:()=>void;onSubmit:(event:PlanningEvent)=>void}){
  ...
  return <div className="overlay" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><form className="dialog event-dialog" role="dialog" aria-modal="true" aria-labelledby="event-title" onSubmit={submit}>
    ...
    {type==='urgent'?<><label>Норматив работы<select value={normId} onChange={e=>{const norm=workNorms.find(item=>item.id===e.target.value);setNormId(e.target.value);if(norm){setDuration(String(norm.totalMinutes));setSkill(norm.skill)}}}><option value="">Ввести длительность вручную</option>{workNorms.map(norm=><option key={norm.id} value={norm.id}>{norm.title} · {norm.totalMinutes} мин</option>)}</select></label>{selectedNorm&&<div className="norm-hint"><b>{selectedNorm.travelMinutes} мин дорога + {selectedNorm.technicalMinutes} мин работы + {selectedNorm.documentsMinutes} мин документы</b><span>Норматив автоматически подставлен в длительность заявки.</span></div>}<label>Адрес<input value={address} onChange={e=>setAddress(e.target.value)} placeholder="Улица и дом" required/></label><div className="form-row"><label>Широта<input type="number" step="any" value={lat} onChange={e=>setLat(e.target.value)} required/></label><label>Долгота<input type="number" step="any" value={lng} onChange={e=>setLng(e.target.value)} required/></label></div><div className="form-row"><label>Время события<input type="time" value={eventTime} onChange={e=>setEventTime(e.target.value)} required/></label><label>Начало окна<input type="time" value={start} onChange={e=>setStart(e.target.value)} required/></label></div><div className="form-row"><label>Конец окна<input type="time" value={end} onChange={e=>setEnd(e.target.value)} required/></label><label>Длительность, минут<input type="number" min="1" value={duration} onChange={e=>{setNormId('');setDuration(e.target.value)}} required/></label></div><div className="form-row"><label>Требуемый навык<select value={skill} onChange={e=>setSkill(e.target.value)}>{skills.map(s=><option key={s}>{s}</option>)}</select></label><label>Транспорт<select value={transport} onChange={e=>setTransport(e.target.value as Transport)}><option value="car">Автомобиль</option><option value="walk">Пешком</option><option value="bike">Велосипед</option><option value="transit">Общественный транспорт</option></select></label></div></>:<><label>Время события<input type="time" value={eventTime} onChange={e=>setEventTime(e.target.value)} required/></label>{type==='cancel'?<label>Заявка<select value={requestId} onChange={e=>setRequestId(e.target.value)}>{data.requests.filter(r=>r.status!=='cancelled').map(r=><option key={r.id} value={r.id}>#{r.id} · {r.location.address}</option>)}</select></label>:<label>Инженер<select value={engineerId} onChange={e=>setEngineerId(e.target.value)}>{data.engineers.filter(e=>e.available).map(e=><option key={e.id} value={e.id}>{e.name} · {transportLabel[e.transport]}</option>)}</select></label>}</>}
